...
**Index** has no sidebar. Hero is a Three.js scene built procedurally in `js/hero3d.js`: three rounded-box "books" (leather + gold gilt, `MeshPhysicalMaterial`) under three-point lighting (warm key with `PCFSoft` shadows, cool teal fill, gold rim spot) on a shadow-catching floor, with `RoomEnvironment` baked into a PMREM env map for real reflections on the gilt. ScrollTrigger rotates the stage and dollies the camera back; idle drift + mouse parallax keep it alive when you're not scrolling. The hero `<video>` was retired in favour of this scene; the mp4 still lives in `public/videos/` for re-use. Trending Volumes uses pinned 3D scroll-stack on desktop. Curated Segments uses arrow-row layout linking to each wing.
...
└── public/
    └── videos/             # Local mp4 assets, served at /videos/*
        ├── 3d-animation-of-the-library-scene-with-books-flyin.mp4
        └── video-1070044246195105.mp4
```
